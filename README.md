...
Choose the `work` or `home` profile for each installation. Cloning these files
needs no account.

## Set up Red Hat Linux

Install Git and Python 3.11 or newer through your package source. Clone
...
anything if a required package is unavailable. Install the missing tools through
a suitable package source and rerun. It does not enable EPEL,
CRB, or Copr, add repositories, or execute remote install scripts. RHEL releases
and enabled repositories differ, so a full setup can require additional
packages.
...
Use a repository-local identity or Git include rules when appropriate.
...
validation. See
[the policy example](copilot/managed-settings.example.json) and
...
Copilot apply its configured sandbox behavior.
...
settings delivered through registry, MDM, or the server are absent. On Windows,
a local policy file needs the appropriate ACLs; the Python check does not
validate ACLs. Package tests and mocked launcher tests do not prove OS containment.
Check `/sandbox status` and `/sandbox policy` in the installed CLI to inspect
the effective policy on that machine.
...
To change a previously installed `home` profile to `work`, use this repository
with `--profile work`. The linker removes
...
Keep common configuration here and environment-specific additions in separate
private overlay repositories. Clone only the overlay needed on that machine.
The public setup never fetches an overlay.
...
After cloning and reviewing an overlay at `~/dotfiles-work`:
...
review it before linking. It does not waive the Copilot launcher's policy checks.
...
credentials in machine-local storage or a credential manager. Private
