#!/usr/bin/env python3
"""Install shared configuration and, when requested, a private home overlay."""
...
class Installer:
    def __init__(self, repo, home, profile, platform, overlay=None):
        ...
        self.records = {}
        self.changes = []
        self.legacy = read_json(overlay / "profiles/legacy.json", {}) if overlay else {}
...
def configured_files(repo, platform):
    manifest = read_json(repo / "profiles/shared.json")
...
def check_home_integrations(installer, powershell_profile=None):
    """Registrations outside our owned files require review before leaving home."""
    if installer.profile != "work" or installer.old.get("profile") != "home":
        return
    import tomllib
        ...
        for directory in ("Library/LaunchAgents", ".config/systemd/user"):
            ...
            for path in root.rglob("*") if root.exists() else ():
                if path.is_file() and overlay.encode() in path.read_bytes():
                    raise ValueError(
                        f"stop and disable the personal service before switching to work: {path}"
                    )
...
def plan(
    ...
    if overlay and profile != "home":
        raise ValueError(
            "private overlays require --profile home; use the shared repo for work"
        )
    ...
    if overlay:
        additions = read_json(overlay / "profiles/home.json")
        files = dict(additions.get("files", {}))
        files.update(additions.get("windows" if platform == "windows" else "unix", {}))
    ...
    if overlay:
        git += "\n[include]\n\tpath = " + json.dumps(
            str(overlay / "gitconfig.home").replace(chr(92), "/")
        )
    ...
    if overlay:
        rules += "\n" + (overlay / "agents/home.md").read_text()
    ...
    if platform == "windows":
        ...
        installer.block(
            ".zshrc",
            exports + f". {shlex.quote(str(repo / 'zshrc'))}",
            legacy_source="zshrc",
        )
        installer.block(
            ".bashrc",
            exports + f". {shlex.quote(str(repo / 'copilot/shell-defaults.sh'))}",
        )
        tmux = "source-file " + json.dumps(str(repo / "tmux.conf"))
        if overlay:
            tmux += "\nsource-file " + json.dumps(str(overlay / "tmux.home.conf"))
